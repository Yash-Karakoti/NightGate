/**
 * CLI for interacting with zk-creator contract
 */
...
const PRIVATE_STATE_ID = 'zkCreatorPrivateState';
...
const compiledContract = CompiledContract.make('zk_creator', ZkCreator.Contract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'zk-creator-state',
      accountId,
...
async function main() {
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Unlock content (ZK proof)');
      console.log('  2. Check total unlocks');
      ...
      switch (choice.trim()) {
        case '1': {
          const secret = await rl.question('  Enter your secret (64-char hex string): ');
          const trimmed = secret.trim();
          if (!/^[0-9a-fA-F]{64}$/.test(trimmed)) {
            console.log('\n  ❌ Invalid input. Please enter a 64-character hex string.\n');
            break;
          }
          console.log('\n  Generating ZK proof and submitting transaction...');
          console.log('  (this may take 30-60 seconds)\n');
          try {
            const secretBytes = Buffer.from(trimmed, 'hex');
            const tx = await deployed.callTx.unlock(secretBytes);
        ...
        case '2': {
          console.log('\n  Reading unlock count from blockchain...');
          try {
            const contractState = await providers.publicDataProvider.queryContractState(deployment.address);
            if (contractState) {
              const ledgerState = ZkCreator.ledger(contractState.data);
              console.log(`\n  🔓 Total unlocks: ${ledgerState.total_unlocks}\n`);
            } else {
